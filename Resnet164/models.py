...
class origin_Bottleneck(nn.Module):
    expansion = 4  

    def __init__(self, inplanes, outplanes, stride=1):
        assert outplanes % self.expansion == 0
        super(origin_Bottleneck, self).__init__()
    ...
    def _make_layer(self):
        ...
        self.bn1 = nn.BatchNorm2d(self.bottleneck_planes)
...
class new_Bottleneck(nn.Module):
    ...
    def forward(self, x):
        residual = x
        out = self.relu(self.bn1(x)) # pre-activation
        out = self.conv1(out)
...
def new_resnet_164(output_classes):
    model = ResNet(new_Bottleneck, 164, output_classes)
    return model

def origin_resnet_164(output_classes):
    model = ResNet(origin_Bottleneck, 164, output_classes)
    return model

if __name__ == '__main__':
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = new_resnet_164(output_classes=10)
    #model = origin_resnet_164(output_classes=10)
